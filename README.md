# XMC MCU: UART Transmit Receive FIFO Interrupts

The code example demonstrates how to use the Tx and Rx FIFO limit interrupts and send data from the Tx buffer to the Rx buffer.

The Tx buffer sends 251 bytes by USIC0_CH1 using the Tx FIFO size of 8 bytes. The data is received in USIC0_CH1 using an Rx FIFO of 8 bytes. If reception is successful, the on-board LED 1 glows; otherwise, the LED remains OFF.
...
4. Verify that the LED turns ON on a successful transmission of data and that the LED remains OFF if the transmission failed.
...
In this code example, the UART peripheral is configured to generate interrupts when Transmit FIFO Limit and Receive FIFO Limit are reached, which are configured to 1 and 7 respectively.
...
The main function is then used to compare the data in the `tx_buffer` with the data in the `rx_buffer`. If they match, LED1 is turned ON suggesting a successful transmission of data. If a mismatch occurs, LED1 remains OFF.
...
**Note:** On the XMC1400 Boot Kit, pins P1.2 and P1.3 are used for UART Tx and Rx respectively, while on the XMC4700 Relax Kit, pins P6.4 and P6.5 are used for UART Tx and Rx.
...
Document Title: *CE232725* - *XMC MCU: UART Transmit Receive FIFO Interrupts*
